...
module.exports = {
    ...
    blogModel.find({}, function (err, blogs) {
      if (err) {
        ...
        for (let blog of blogs) {
          blogList.push({ id: blog._id, title: blog.title, uri: blog.uri , published_on: blog.published_on, content: blog.content, description: blog.description });
        }
  ...
  updateById: function (req, res, next) {
    blogModel.findByIdAndUpdate(req.params.blogId, { title: req.body.title, uri: req.body.uri, description: req.body.description, content: req.body.content }, function (err, blogInfo) {
      if (err)
        next(err);
      else {
  ...
  create: function (req, res, next) {
    blogModel.create({ title: req.body.title, uri: req.body.uri, description: req.body.description, content: req.body.content }, function (err, result) {
      if (err)
        next(err);
      else
